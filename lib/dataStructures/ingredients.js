...
export const checkIngredient = (ingredient) => {
  const errors = [];
  if (ingredient.name == "") {
    errors.push({errorType: 'EmptyFieldError', field: 'Ingredient name'});
  }
  if (ingredient.amount == "") {
    errors.push({errorType: 'EmptyFieldError', field: 'Ingredient amount'});
  }
  if (isNaN(+ingredient.amount)) {
    errors.push({errorType: 'NumericError', field: "Ingredient amount"})
  }
  return(errors);
}